...
class NeuralCLBFController(pl.LightningModule, CLFController):
    ...
    def __init__(
        ...
        self.input_normalizer = nn.BatchNorm1d(dynamics_model.n_dims)
    ...
    def solve_optimization(self, problem):
        try:
            result = problem.solve(solver=self.solver, **self.solver_opts)
            return result
        except cp.error.SolverError:
            print("ECOS solver failed. Trying alternative solver.")
            return problem.solve(solver=cp.SCS, **self.solver_opts)
    ...
    def forward(self, x):
        x_normalized = self.input_normalizer(x)
        return self.u(x_normalized)
