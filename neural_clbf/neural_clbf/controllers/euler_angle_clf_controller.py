...
class CLFController(Controller):
    ...
    def _solve_CLF_QP_cvxpylayers(
        ...
        #     print(f"V: {V}, LfV: {LfV}, LgV: {LgV}")
        #     print(f"h: {h}, Lfh: {Lfh}, Lgh: {Lgh}")
        #     print(f"Q: {Q}, p: {p}")
        #     print(f"G: {G}, h_qp: {h_qp}")
        #     print(f"A: {A}, b: {b}")
        #     raise
        ...
        try:
            ...
            print(f"Input state x: {x}")
            print(f"u_ref: {u_ref}")
            print(f"V: {V}, Lf_V: {Lf_V}, Lg_V: {Lg_V}")
            print(f"relaxation_penalty: {relaxation_penalty}")
            print(f"params: {params}")
            # if isinstance(Lg_V, torch.Tensor):
            #     Lg_V_np = Lg_V.detach().cpu().numpy()
            #     if np.any(np.all(Lg_V_np == 0, axis=1)):
            #         print("Warning: Lg_V contains all-zero rows, which may indicate an unbounded problem.")
            
            # if isinstance(u_ref, torch.Tensor):
            #     u_ref_np = u_ref.detach().cpu().numpy()
            #     if np.any(np.isinf(u_ref_np)) or np.any(np.isnan(u_ref_np)):
            #         print("Warning: u_ref contains inf or nan values, which may lead to an unbounded problem.")
            
            # print(f"params: {params}")
            raise
            # return torch.zeros_like(u_ref), torch.full_like(V, relaxation_penalty)
