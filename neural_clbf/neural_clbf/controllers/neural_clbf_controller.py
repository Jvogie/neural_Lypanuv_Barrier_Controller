...
class NeuralCLBFController(pl.LightningModule, CLFController):
    ...
    def __init__(
        ...
        # Adjust this weight to balance CLBF and reactive avoidance
        # self.obstacle_avoidance_weight = 0.5
    ...
    def descent_loss(
        ...
        # Add obstacle proximity penalty
        obstacle_proximity_penalty = torch.tensor(0.0).type_as(x)
        for obstacle_pos, obstacle_radius in self.dynamics_model.obstacles:
            distances = torch.norm(x[:, :3] - obstacle_pos.to(x.device), dim=1)
            proximity = F.relu(self.dynamics_model.obstacle_buffer * 2 - distances)  # Penalize being within 2x buffer distance
            obstacle_proximity_penalty += proximity.mean()

        # Scale the penalty and add it to the loss
        obstacle_penalty_weight = 200.0  # Increased from previous value
        obstacle_proximity_loss = obstacle_penalty_weight * obstacle_proximity_penalty
        loss.append(("Obstacle Proximity Penalty", obstacle_proximity_loss))
