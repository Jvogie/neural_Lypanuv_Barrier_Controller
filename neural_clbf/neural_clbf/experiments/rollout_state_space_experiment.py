...
class RolloutStateSpaceExperiment(Experiment):
    ...
    def run(self, controller_under_test: "Controller") -> pd.DataFrame:
        ...
        if not self.scenarios:
            self.scenarios = [controller_under_test.dynamics_model.nominal_params]
        ...
        for i in range(n_sims):
            ...
            for param_name in self.scenarios[0].keys():
                ...
                if isinstance(param_min, (int, float)) and isinstance(param_max, (int, float)):
                    ...
                    random_scenario[param_name] = self.scenarios[0][param_name]  # Use the first scenario's value as default
                ...
                for param_name, param_value in random_scenarios[sim_index].items():
                    if isinstance(param_value, torch.Tensor):
                        if param_value.numel() == 1:
                            param_value_string = "{:.3g}".format(param_value.item())
                        else:
                            param_value_string = str(param_value.tolist())
                    else:
